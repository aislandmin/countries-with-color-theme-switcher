...
import { requestCountriesByCodes } from "../api/CountryService";
...
export const CountryInfo = (props) => {
  ...
  useEffect(() => {
    const getNeighborsByCodes = async (codes) => {
      if ( !codes || codes.length === 0) return;

      const neighborCountries = await requestCountriesByCodes(codes);
      const countryNames = neighborCountries.map((item) => item.name.common);

      setNeighbors(countryNames);
    };

    getNeighborsByCodes(props.borders);
  }, [props.borders]);

  const curName = Object.values(props.currencies);
  const langs = Object.values(props.languages);
  const nativeNames = Object.values(props.name.nativeName);
